...
types <- c("full", "b_sel", "varsel")
noise <- c(0.1, 0.3, 0.5)
ntrials <- 10
design <- expand.grid(
  mc = mc, noise = noise, 
  trial = 1:ntrials, type = types
)
...
results <- foreach(i = seq_len(nrow(design)),
  ...
  out <- design[i, ]
  suffix <- paste0("mc", out$mc, "_noise", out$noise, "_trial", out$trial)
  if (out$type != "full") {
   suffix <- paste0(suffix, "_", out$type)
  }
  fit <- readRDS(paste0("models/noise/fit_ishigami_", suffix, ".rds"))
...
saveRDS(results, "ishigami_summary_noise.rds")
